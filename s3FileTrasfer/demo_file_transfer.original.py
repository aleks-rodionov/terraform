...
class TransferDemoManager:
    ...
    def collect_user_info(self):
        ...
        while not self.demo_folder:
            ...
            if not os.path.isdir(self.demo_folder):
                print("{} isn't a folder!".format(self.demo_folder))
                self.demo_folder = None
        ...
        while not self.demo_bucket:
            ...
            try:
                ...
                print(err)
                print(
                    "Either demo_bucket doesn't exist or you don't "
                    "have access to it.")
                self.demo_bucket = None
    ...
    def cleanup(self):
        ...
        for local_file_path, s3_object_key, downloaded_file_path \
                in self._chore_list:
            print("Removing {}".format(local_file_path))
            try:
                os.remove(local_file_path)
            except FileNotFoundError as err:
                print(err)

            print("Removing {}".format(downloaded_file_path))
            try:
            ...
            if self.demo_bucket:
                print("Removing {}:{}".format(self.demo_bucket,s3_object_key))
                try:
    ...
    def _setup_platform_specific(self):
        ...
        if platform.system() == "Windows":
            ...
            raise EnvironmentError(
                "Demo of platform {} isn't supported.".format(platform.system()))
    ...
    def _create_demo_file(self):
        ...
        print("Creating file of size {} MB ".format(self.file_size_mb)
              "in {} by running:".format(self.demo_folder))
        print("{'':4}{}".format(filled_cmd))
    ...
    def _report_transfer_params(self, verb, source_name, dest_name, **kwargs):
        ...
        print('-' * self._terminal_width)
        print('{} {} ({} MB) to {}'.format(verb,source_name,self.file_size_mb,dest_name))
        if kwargs:
            print('With extra args:')
            for arg, value in kwargs.items():
                print('{"":4}{arg:<20}: {}'.format(value))
    ...
    def _config_wrapper(func, config_attrs):
        def wrapper(*args, **kwargs):
            ...
            for attr in config_attrs:
                print('{"":4}{attr:<20}: {}'.format(getattr(config, attr)))
            return config
    ...
    def _report_transfer_result(thread_info, elapsed):
        ...
        print("\nUsed {} threads.".format(len(thread_info)))
        for ident, byte_count in thread_info.items():
            print("{'':4}Thread {} copied {} bytes.".format(ident,byte_count))
        print("Your transfer took {} seconds.".format(elapsed:.2f))
