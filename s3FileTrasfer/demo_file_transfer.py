...
class TransferDemoManager:
    ...
    def run_interactive(self):
        ...
        if self.ask_user("Do you want to try to download the encrypted "
    ...
    def process_command_line(self):
        ...
        self.demo_folder = sys.argv[1]
        self.demo_bucket = sys.argv[2]
        ...
        try:
            self.file_size_mb = os.stat(upload_file_name).st_size
        except ClientError as err:
            print(err)
            print(
                f"Either {upload_file_name} doesn't exist or you don't "
                f"have access to it.")
            self.demo_folder = None

#        file_transfer.TransferConfig = \
#            self._config_wrapper(TransferConfig, CONFIG_ATTRS)
#        print(self._config_wrapper)
#        self._report_transfer_params('Uploading', upload_file_name, "", "")

        upload_with_default_configuration(local_file_path, self.demo_bucket,
                                          object_key, self.file_size_mb)
...
def main():
    ...
    arguments = len(sys.argv) - 1
    if arguments > 0:
        print("start process_command_line")
        demo_manager.process_command_line()
        print("end process_command_line")
    else:
        demo_manager.run_interactive()
